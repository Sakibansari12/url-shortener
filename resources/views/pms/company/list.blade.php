    ...
    <div class="container-fluid">
        <div class="title">
            <div class="row gx-2 align-items-center">
                <div class="col">
                    <h1 class="fs-5 mb-0">Company List</h1>
                </div>
                <div class="col-auto">
                    <a href="{{ route('pms.company.form') }}" class="btn d-flex btn-small rounded-2 btn-secondary">
        ...
        <div class="content-box p-3">
            <form method="GET" action="{{ route('pms.company.list') }}">
                
                <div class="row mb-3 g-3 flex-sm-row-reverse align-items-center">
                    <div class="col-sm-6 col-md-5 col-lg-4">
                        <div class="input-group input-group-sm">
                            <input type="text" name="search" class="form-control" placeholder="Search by company." value="{{ request('search') }}">
                            <button type="submit" class="btn btn-primary">
                                <i class="icon-search"></i>
                            </button>
                            <a href="{{ route('pms.company.list') }}" class="btn btn-warning">
                ...
                <table class="table align-middle">
                    ...
                    <tbody id="sortable">
                        ...
                        <tr data-id="{{ $value->id }}">
                            ...
                            <td>
                                <ul class="actions">
                                    <li><a href="{{ route('pms.company.form', ['id'=>$value->id]) }}" class="btn btn-link"><i class="icon-edit"></i></a></li>
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        ...
        $(".deleteSelected").click(function () {
            ...
            if (idArray.length > 0) {
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        $.ajax({
                            url: '{{ url("pms/company/multidelete") }}',
                            method: 'POST',
                            data: { ids: idArray },
                            headers: {
                                'X-CSRF-TOKEN': $('meta[name="_token"]').attr('content')
        ...
        $('.btn_status').on('click', function () {
            ...
            $.ajax({
                url: '{{ url("pms/company/toggle-status") }}/' + id,
                type: 'PUT',
                headers: {
                    'X-CSRF-TOKEN': $('meta[name="_token"]').attr('content')
                },
