...
document.addEventListener("DOMContentLoaded", function () {
    ...
    if (typeof HotelDatepicker === 'undefined') {
      console.error('HotelDatepicker is not loaded!');
      return;
    }
    ...
    const datepicker = new HotelDatepicker(input, {
      ...
      onSelectRange: function () {
        const [start, end] = datepicker.getValue().split(' - ');
  
        const formattedStart = fecha.format(new Date(start), 'D MMM YYYY');
        const formattedEnd = fecha.format(new Date(end), 'D MMM YYYY');
  
        document.getElementById("output").innerText = `Selected: ${formattedStart} → ${formattedEnd}`;
      }
    });
  });
